...
const connection =  new ewelink({ email: process.env.EVELINK_EMAIL || '', password: process.env.EVELINK_PASSWORD || '' });
...
app.post('/:id/on', async (req, res, next)  => {
    console.log(`Received request on for ${req.params.id}`);
...
app.post('/:id/off', async (req, res, next)  => {
    console.log(`Received request off for ${req.params.id}`);
    const device = await connection.getDevice(req.params.id);
